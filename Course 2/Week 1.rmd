...
samp1 %>%
  summarise(mu_samp = mean(area), pop_med_samp = median(area), 
            sigma_samp = sd(area), pop_iqr_samp = IQR(area),
            pop_min_samp = min(area), pop_max_samp = max(area),
            pop_q1_samp = quantile(area, 0.25),  # first quartile, 25th percentile
            pop_q3_samp = quantile(area, 0.75))  # third quartile, 75th percentile
```
...
sample_means_small <- ames %>%
  ...
  summarise(x_bar = mean(area))
ggplot(data = sample_means_small, aes(x = x_bar)) + geom_histogram()
```
...
# type your code for this Exercise here, and Run Document
sample_means50 <- ames %>%
  rep_sample_n(50,reps = 5000,replace = TRUE) %>%
...
sample_means150 <- ames %>%
  rep_sample_n(150, reps = 5000, replace = TRUE) %>%
  summarise(x_bar = mean(price))
ggplot(sample_means150,aes(x_bar)) +
  geom_histogram(bins = 50)
```
...
sample_means15 <- ames %>%
  ...
  summarise(x_bar = mean(price))
ggplot(data = sample_means15, aes(x = x_bar)) + geom_histogram(binwidth = 4000)
...
sample_means150 <- ames %>%
  ...
  summarise(x_bar = mean(price))
ggplot(data = sample_means150, aes(x = x_bar)) + geom_histogram(bins = 50)
