...
ci_data <- ci %>%
  gather("lower","upper",key = "ci_id",value = "ci_bounds",-replicate)
```

And finally we can create the plot using the following:

```{r plot-ci}
ggplot(ci_data,aes(ci_bounds,replicate,group = replicate, color = capture_mu)) +
  geom_line() +
  geom_point(size = 2) +
...
# type your code for the Exercise here, and Knit
ci <- ci %>%
  mutate(lower_99 = lower - (2.58-1.96) * (lower / 1.96),
         higher_99 = upper + (2.58-1.96) * (upper / 1.96),
         capture_99_mu = ifelse(lower_99 < params$mu & higher_99
                                > params$mu, "yes", "no"))
ci_99_data <- data.frame(ci_id = c(1:50, 1:50),
                         ci_bounds = c(ci$lower_99, ci$higher_99),
                         capture_mu = c(ci$capture_99_mu, ci$capture_99_mu))
